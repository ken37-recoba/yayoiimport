...
function callGeminiApi(fileBlob, prompt) {
  ...
  try {
    ...
    // ▼▼▼【修正箇所】サポートされなくなった thinkingConfig を削除 ▼▼▼
    const payload = {
      "contents": [{
        "parts": [
          { "text": prompt },
...
function getPassbookGeminiPrompt(bankType) {
    ...
    const basePrompt = `
# 役割
あなたは、日本の銀行通帳の読取りに特化した高精度OCRエンジンです。あなたの任務は、提供された通帳の画像から取引履歴を正確に抽出し、指定されたJSON形式のオブジェクトとして結果を返すことです。
...
# 検算ルール (重要)
- 各取引行の残高は、**[前の行の残高] - [出金額] + [入金額]** と一致するはずです。この計算式と照らし合わせ、入金（残高が増える取引）と出金（残高が減る取引）を絶対に間違えないでください。
- もし、どうしてもこの計算が合わない場合は、その矛盾を**「備考」欄に「【要確認：残高不整合】」**と記録してください。

# 文字整形ルール (最重要)
- **カタカナ:** 半角カタカナは全角カタカナに変換してください。**ひらがなや漢字は変換しないでください。**
- **英数字:** すべて**半角**で出力してください。（例: ＤＦ -> DF, １２３ -> 123）
- **濁点・半濁点の結合:** 濁点(ﾞ)や半濁点(ﾟ)は、前の文字と結合し、必ず1文字として表現してください。（例: 「フ」と「ﾞ」ではなく、「ブ」として出力）
...
# 除外ルール (重要)
- 「摘要」が「繰越」や「繰越残高」となっており、かつ「お支払金額」と「お預り金額」の両方が空欄または0の行は、実際の取引ではないため、出力に**含めないでください**。
- 同様に、「摘要」が完全に空欄で、かつ「お支払金額」と「お預り金額」の両方が0の行も、ページ先頭の繰越残高行とみなし、出力に**含めないでください**。
...
# JSON出力形式 (最重要)
- 必ず以下の構造の**単一のJSONオブジェクト**として出力してください。
\`\`\`json
{
  "transactions": [
    {
...
\`\`\`
- \`transactions\`は取引データの配列です。
- \`rowCount\`は、画像から読み取った取引行の**合計数**を数値で記載してください。
`;
